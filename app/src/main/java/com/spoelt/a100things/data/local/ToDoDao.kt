...
interface ToDoDao {
    ...
    @Query("SELECT * FROM to_do_list_items")
    fun observeAllToDoListItems(): LiveData<ToDoListItem>

    @Query("SELECT * FROM to_do_list_items WHERE isComplete = 1")
    fun observeCompletedToDoListItems(): LiveData<ToDoListItem>

    @Query("SELECT * FROM to_do_list_items WHERE isComplete = 0")
    fun observeOpenToDoListItems(): LiveData<ToDoListItem>
}